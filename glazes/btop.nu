...
def get-manifest []: nothing -> record {
    ...
    (build-manifest
        ...
        --scope $SCOPE.linux
        --toppings [
            { name: 'btop' url: 'https://github.com/aristocratos/btop' os: $OS.linux package_manager: 'pacman' }
        ]
...
def do-config []: nothing -> bool {
    ...
    let target_dir: directory = config-dir -m btop

    log $MESSAGE.io_info_install_config
    let source_config_path: path = $linux_dir | path join 'btop.conf'
    let target_config_path: path = $target_dir | path join 'btop.conf'
