...
def get-manifest []: nothing -> record {
    ...
    (build-manifest
        ...
        --scope $SCOPE.common
        --toppings [
            { name: 'fnm-bin' url: 'https://github.com/Schniz/fnm' os: $OS.linux package_manager: 'paru' }
            { name: 'Schniz.fnm' url: 'https://github.com/Schniz/fnm' os: $OS.windows package_manager: 'winget' }
        ]
...
def do-config []: nothing -> bool {
    ...
    let source_config_path: path = $common_dir | path join 'fnm.nu'
    let target_config_path: path = $target_autoload_dir | path join 'fnm.nu'
