...
def get-manifest []: nothing -> record {
    ...
    (build-manifest
        ...
        --scope $SCOPE.common
        --toppings [
            { name: 'zoxide' url: 'https://github.com/ajeetdsouza/zoxide' os: $OS.linux package_manager: 'pacman' }
            { name: 'ajeetdsouza.zoxide' url: 'https://github.com/ajeetdsouza/zoxide' os: $OS.windows package_manager: 'winget' }
        ]
...
def do-config []: nothing -> bool {
    ...
    let source_config_path: path = [$common_dir nushell] | path join 'zoxide.nu'
    add-environment $ID (open $source_config_path)
}
