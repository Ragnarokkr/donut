...
def get-manifest []: nothing -> record {
    (build-manifest
        ...
        --scope $SCOPE.windows
        --toppings [
            { name: 'krokiet' url: 'https://github.com/qarmin/czkawka/tree/master/krokiet' os: $OS.windows package_manager: 'custom' }
        ]
    )
}

def do-install []: nothing -> bool {
    let target_path: path = local-bin-dir -m | path join 'krokiet.exe'

    github get qarmin czkawka 'windows_krokiet_on_linux.exe'
    | save -fr $target_path

    $target_path | path exists
}
