...
def get-manifest []: nothing -> record {
    ...
    (build-manifest
        ...
        --scope $SCOPE.linux
        --toppings [
            { name: 'curl' url: 'https://curl.se' os: $OS.linux package_manager: 'pacman' }
        ]
...
def do-config []: nothing -> bool {
    ...
    let source_config_path: path = $linux_dir| path join 'curl.rc'
    let target_config_path: path = $target_dir | path join '.curl.rc'
