...
def get-manifest []: nothing -> record {
    ...
    (build-manifest
        ...
        --scope $SCOPE.linux
        --toppings [
            { name: 'gdb' url: 'https://www.sourceware.org/gdb/' os: $OS.linux package_manager: 'pacman' }
        ]
...
def do-config []: nothing -> bool {
    let linux_dir: directory = work-dir $ID | get $SCOPE.linux
    let target_dir: directory = home-dir -m gdb

    log $MESSAGE.io_info_install_config
    let source_config_path: path = $linux_dir | path join 'gdbinit'
    let target_config_path: path = $target_dir | path join 'gdbinit'
