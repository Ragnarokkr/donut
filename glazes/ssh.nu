...
def get-manifest []: nothing -> record {
    ...
    (build-manifest
        ...
        --toppings [
            { name: 'openssh' url: 'https://www.openssh.org/' os: $OS.linux package_manager: 'pacman' }
        ]
...
def do-config []: nothing -> bool {
    let linux_dir: directory = work-dir $ID | get $SCOPE.linux
    let target_dir: directory = home-dir -m .ssh

    log $MESSAGE.io_info_install_config
    let source_config_path: path = $linux_dir | path join 'config'
    let target_config_path: path = $target_dir | path join 'config'
