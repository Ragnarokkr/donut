...
- (Lin) #link("https://www.sudo.ws/")[sudo]
- (Lin) #link("https://github.com/Duncaen/OpenDoas")[doas] (optional, but it takes precedence over `sudo` if installed)
...
- *Phase 1*: Preinstall and preconfig
  ...
  - *Preinstall*: Installs essential system-wide tools (e.g., `paru`, `unzip`).
  - *Preconfig*: Initial configuration for the preinstalled tools.
  - *Restart*: A mandatory pause. Shells or OS restarts ensure new binaries
...
- *Phase 2*: Install and config
  ...
  - *Second Restart*: The final _bake_. Ensures all changes are active and stable.
